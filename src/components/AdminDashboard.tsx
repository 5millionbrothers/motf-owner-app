...
export default function AdminDashboard({ supabase, onLogout }: { supabase: SupabaseClient; onLogout: () => void }) {
  ...
  const [marketOrders, setMarketOrders] = useState<Row[]>([]);
  ...
  const loadAll = useCallback(async () => {
    setNotice("");
    const [businessResult, profileResult, reservationResult, marketOrderResult, paymentIntentResult, conversationResult, caseResult, reviewResult, postResult] = await Promise.all([
      ...
      supabase.from("payment_intents").select("order_id,customer_id,kind,amount,order_name,draft,status,virtual_account,virtual_account_issued_at,created_at").eq("status","virtual_account_issued").order("created_at", { ascending:false }),
      supabase.from("conversations").select("*").order("last_message_at", { ascending:false }),
      supabase.from("support_cases").select("*").order("created_at", { ascending:false }),
      supabase.from("reviews").select("*").order("created_at", { ascending:false }),
      supabase.from("community_posts").select("*").order("created_at", { ascending:false }),
    ]);
    setBusinesses(businessResult.data || []); setProfiles(profileResult.data || []); setReservations(reservationResult.data || []); setMarketOrders(marketOrderResult.data || []); setPaymentIntents(paymentIntentResult.data || []); setConversations(conversationResult.data || []); setCases(caseResult.data || []); setReviews(reviewResult.data || []); setPosts(postResult.data || []);
  ...
  const confirmedReservations = reservations.filter((item) => ["confirmed","completed"].includes(item.status));
  const confirmedMarketOrders = marketOrders.filter((item) => ["confirmed","completed"].includes(item.status));
  const totalRevenue = [...confirmedReservations, ...confirmedMarketOrders].reduce((sum,item) => sum + Number(item.total_amount || 0), 0);
  const pendingIntentAmount = paymentIntents.reduce((sum,item) => sum + Number(item.amount || 0), 0);
  const businessName = (id:string) => businesses.find((item) => item.id === id)?.business_name || "업장 정보 없음";
  const pendingForBusiness = (businessId:string) => paymentIntents.filter((item) => item.draft?.business_id === businessId);
  const confirmedAmountForBusiness = (businessId:string) => [
    ...confirmedReservations.filter((item) => item.business_id === businessId),
    ...confirmedMarketOrders.filter((item) => item.business_id === businessId),
  ].reduce((sum,item) => sum + Number(item.total_amount || 0), 0);
  const businessRate = (businessType:string) => businessType === "market" ? .05 : .07;
  ...
  return <main className="dashboard-page admin-dashboard">
    ...
    <section className="dashboard-content"><header className="dashboard-top"><button className="admin-logout-button" onClick={onLogout}>로그아웃</button></header><div className="dashboard-body">{notice&&<div className="dashboard-notice">{notice}</div>}
      {menu==="stats"&&<section><h2 className="owner-panel-title">moTF 플랫폼 마스터 실시간 종합 현황</h2><div className="owner-stats-grid"><Stat label="가입 파트너" value={`${partnerProfiles.length}명`}/><Stat label="입점 심사 대기" value={`${partnerProfiles.filter((p)=>p.status==="pending").length}건`}/><Stat label="플랫폼 누적 거래액" value={`${totalRevenue.toLocaleString()}원`}/><Stat label="입금 대기 거래액" value={`${pendingIntentAmount.toLocaleString()}원`}/></div><AdminTable headers={["업장","예약자","이용일","금액","상태"]} rows={reservations.slice(0,10).map((r)=><tr key={r.id}><td>{businessName(r.business_id)}</td><td>{r.customer_name}</td><td>{r.event_date}</td><td>{r.total_amount.toLocaleString()}원</td><td><Badge text={reservationLabel[r.status]}/></td></tr>)}/></section>}
      ...
      {menu==="reservations"&&<section><h2 className="owner-panel-title">운영팀 예약 관리</h2>{paymentIntents.length>0&&<><h3 className="section-subtitle">가상계좌 입금 대기</h3><div className="reservation-list">{paymentIntents.map((p)=><article key={p.order_id}><div><small>{businessName(p.draft?.business_id)}</small><h3>{p.draft?.customer_name || "이용자"}</h3><p>{p.kind==="stay" ? p.draft?.event_date : String(p.draft?.pickup_time || "").slice(0,5)} | {p.order_name} | {Number(p.amount).toLocaleString()}원</p></div><Badge text="입금 대기"/></article>)}</div></>}{reservations.length?<div className="reservation-list">{reservations.map((r)=><article key={r.id}><div><small>{businessName(r.business_id)}</small><h3>{r.customer_name}</h3><p>{r.event_date} | {r.offering_name} | {r.total_amount.toLocaleString()}원</p></div>{r.status==="pending"?<div><button className="owner-pill-button approve" onClick={()=>setReservation(r.id,"confirmed")}>운영팀 확정</button><button className="owner-pill-button reject" onClick={()=>setReservation(r.id,"rejected")}>운영팀 거절</button></div>:<Badge text={reservationLabel[r.status]}/>}</article>)}</div>:<Empty text="확정 처리할 예약이 없습니다."/>}</section>}
      ...
      {menu==="revenue"&&<section><h2 className="owner-panel-title">moTF 본사 종합 매출 분석</h2><div className="owner-stats-grid"><Stat label="전체 거래액" value={`${totalRevenue.toLocaleString()}원`}/><Stat label="입금 대기액" value={`${pendingIntentAmount.toLocaleString()}원`}/><Stat label="예상 수수료" value={`${businesses.reduce((sum,b)=>sum+Math.floor(confirmedAmountForBusiness(b.id)*businessRate(b.business_type)),0).toLocaleString()}원`}/><Stat label="확정 거래" value={`${confirmedReservations.length + confirmedMarketOrders.length}건`}/></div></section>}
      {menu==="settlement"&&<section><h2 className="owner-panel-title">본사 월별 대금 정산 통제 센터</h2><AdminTable headers={["정산 대상 파트너","업종 요율","총 매출액","입금 대기액","수수료","최종 입금 예정액"]} rows={businesses.map((b)=>{const amount=confirmedAmountForBusiness(b.id);const pending=pendingForBusiness(b.id).reduce((s,item)=>s+Number(item.amount||0),0);const rate=businessRate(b.business_type);return <tr key={b.id}><td><strong>{b.business_name}</strong></td><td>{rate*100}%</td><td>{amount.toLocaleString()}원</td><td>{pending.toLocaleString()}원</td><td>{Math.floor(amount*rate).toLocaleString()}원</td><td>{Math.floor(amount*(1-rate)).toLocaleString()}원</td></tr>})}/></section>}
    </div></section>
  </main>;
}
