...
export default function AdminDashboard({ supabase, onLogout }: { supabase: SupabaseClient; onLogout: () => void }) {
  ...
  const [recreationSubmissions, setRecreationSubmissions] = useState<Row[]>([]);
  const [selectedBusiness, setSelectedBusiness] = useState("");
  const [selectedConversation, setSelectedConversation] = useState("");
  const [messages, setMessages] = useState<Row[]>([]);
  const [notice, setNotice] = useState("");
  const [contentTab, setContentTab] = useState<"reviews"|"posts"|"recreation">("reviews");

  const loadAll = useCallback(async () => {
    setNotice("");
    const [businessResult, profileResult, reservationResult, conversationResult, caseResult, reviewResult, postResult, recreationResult] = await Promise.all([
      ...
      supabase.from("recreation_submissions").select("*").order("created_at", { ascending:false }),
    ]);
    setBusinesses(businessResult.data || []); setProfiles(profileResult.data || []); setReservations(reservationResult.data || []); setConversations(conversationResult.data || []); setCases(caseResult.data || []); setReviews(reviewResult.data || []); setPosts(postResult.data || []); setRecreationSubmissions(recreationResult.data || []);
  ...
  async function reviewRecreation(id:string, review_status:"approved"|"rejected") { const {data:user}=await supabase.auth.getUser(); const {error}=await supabase.from("recreation_submissions").update({review_status,reviewed_by:user.user?.id,reviewed_at:new Date().toISOString()}).eq("id",id); setNotice(error?error.message:"레크레이션 추천 검토 상태를 변경했습니다."); await loadAll(); }
  ...
  return <main className="dashboard-page admin-dashboard">
    ...
    <section className="dashboard-content"><header className="dashboard-top"><button className="admin-logout-button" onClick={onLogout}>로그아웃</button></header><div className="dashboard-body">{notice&&<div className="dashboard-notice">{notice}</div>}
      {menu==="stats"&&<section><h2 className="owner-panel-title">moTF 플랫폼 마스터 실시간 종합 현황</h2><div className="owner-stats-grid"><Stat label="가입 파트너" value={`${partnerProfiles.length}명`}/><Stat label="입점 심사 대기" value={`${partnerProfiles.filter((p)=>p.status==="pending").length}건`}/><Stat label="플랫폼 누적 거래액" value={`${totalRevenue.toLocaleString()}원`}/><Stat label="본사 예상 수수료" value={`${Math.floor(totalRevenue*.035).toLocaleString()}원`}/></div><AdminTable headers={["업장","예약자","이용일","금액","상태"]} rows={reservations.slice(0,10).map((r)=><tr key={r.id}><td>{businessName(r.business_id)}</td><td>{r.customer_name}</td><td>{r.event_date}</td><td>{r.total_amount.toLocaleString()}원</td><td><Badge text={reservationLabel[r.status]}/></td></tr>)}/></section>}
