(function setupStayWebsiteImporter() {
  ...
  function renderEditor() {
    ...
    $("#stayImportEditor").innerHTML = `<form id="stayImportCommitForm" class="stay-import-editor">
      <section class="stay-import-block"><div class="stay-import-block-title"><div><strong>1. 기본 정보</strong><span>AI가 비워둔 값은 근거가 없다는 뜻입니다.</span></div></div><div class="stay-import-fields">
        ${field("숙소명", "businessName", draft.businessName)}${field("대표자", "representativeName", draft.representativeName)}${field("전화번호", "phone", draft.phone)}${field("운영 지역", "region", draft.region)}${field("우편번호", "postalCode", draft.postalCode)}${field("상세주소", "addressDetail", draft.addressDetail)}${field("주소", "address", draft.address, { wide:true })}${field("목록 한 줄 소개", "shortDescription", draft.shortDescription, { wide:true })}<label class="wide"><span>상세 소개</span><textarea name="description" rows="7">${escapeHtml(draft.description)}</textarea></label>
      </div></section>
  ...
  async function commit(event) {
    ...
    if (!confirm(`${draft.businessName || "이름 미확인 숙소"}을 비공개 초안으로 등록할까요? 이용자 화면에는 아직 노출되지 않습니다.`)) return;
    ...
    try {
      ...
      $("#stayImportEditor").innerHTML = `<section class="stay-import-complete"><i data-lucide="circle-check-big"></i><div><h2>비공개 초안 생성 완료</h2><p>${escapeHtml(draft.businessName || "숙소명 확인 필요")} · 객실 ${draft.rooms.length}개 · 사진 ${result.imageCount}장</p></div><dl><div><dt>사장님 로그인 이메일</dt><dd><code>${escapeHtml(result.account.email)}</code></dd></div><div><dt>임시 비밀번호</dt><dd><code>${escapeHtml(result.account.password)}</code></dd></div></dl><strong>비밀번호는 서버나 DB에 따로 저장되지 않으므로 지금 안전한 곳에 기록해주세요.</strong>${result.warnings?.length ? `<ul>${result.warnings.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : ""}</section>`;
