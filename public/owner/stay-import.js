(function setupStayWebsiteImporter() {
  ...
  function renderEditor() {
    ...
    $("#stayImportEditor").innerHTML = `<form id="stayImportCommitForm" class="stay-import-editor">
      ...
      <section class="stay-import-block"><div class="stay-import-block-title"><div><strong>3. 객실과 요금</strong><span>0원 객실은 등록되지 않습니다. 1박 기준인지 꼭 확인하세요.</span></div><button type="button" class="secondary-btn compact" id="stayImportAddRoom"><i data-lucide="plus"></i> 객실 추가</button></div><div id="stayImportRooms"></div></section>
      ...
      <section class="stay-import-block"><div class="stay-import-block-title"><div><strong>5. 가져올 사진</strong><span>체크된 사진만 모티프 저장소에 복사합니다. 최대 15장입니다.</span></div></div><div class="stay-import-images">${draft.imageUrls.map((url, index) => `<label><input type="checkbox" data-image-url="${escapeHtml(url)}" ${index < 12 ? "checked" : ""}><img src="${escapeHtml(url)}" alt="가져오기 후보 ${index + 1}" loading="lazy"><span>${index + 1}</span></label>`).join("") || "<p>자동으로 찾은 사진이 없습니다.</p>"}</div></section>
    ...
    $("#stayImportAddRoom").onclick = () => { draft.rooms.push({ name:"새 객실", description:null, price:0, minPeople:null, basePeople:null, maxPeople:null, extraPersonFee:0, bathroomCount:0, bathroomGenderSeparated:false, bathroomNote:null, featureSummary:[] }); renderRooms(); };
  ...
  function readEditor(form) {
    ...
    return { ...state.draft, businessName:value("businessName"), representativeName:value("representativeName"), phone:value("phone"), region:value("region"), postalCode:value("postalCode"), address:value("address"), addressDetail:value("addressDetail"), shortDescription:value("shortDescription"), description:value("description"), rooms, extraFees, facilities, imageUrls:[...form.querySelectorAll("[data-image-url]:checked")].slice(0, 15).map((input) => input.dataset.imageUrl) };
  }
